...
class BackTestStock:
    ...
    def backtest_KD_pick(self, mainParament: RecordBackTestParameter):
        ...
        Temp_table = TGetExternalData().get_stock_history(
        ...
        for index, row in Temp_table.iterrows():
            ...
            if ROE_record_day <= index:
                ...
                ROE_data["ROE_data_1"] = pd.DataFrame(
                    get_ROE_range(changeDateMonth(index, -3), 0, 999)
                )
                ROE_data["ROE_data_1"].rename(
                    columns={"ROE": "ROE_data_1"}, inplace=True
                )
                ROE_data["ROE_data_2"] = pd.DataFrame(
                    get_ROE_range(changeDateMonth(index, -6), 0, 999)
                )
                ROE_data["ROE_data_2"].rename(
                    columns={"ROE": "ROE_data_2"}, inplace=True
                )
                ROE_data["ROE_data_3"] = pd.DataFrame(
                    get_ROE_range(changeDateMonth(index, -9), 0, 999)
                )
                ROE_data["ROE_data_3"].rename(
                    columns={"ROE": "ROE_data_3"}, inplace=True
                )
                ROE_data["ROE_data_4"] = pd.DataFrame(
                    get_ROE_range(changeDateMonth(index, -12), 0, 999)
                )
                ROE_data["ROE_data_4"].rename(
                    columns={"ROE": "ROE_data_4"}, inplace=True
                )
                ...
                ROE_data_1 = pd.Series(mask["ROE_data_1"])
                ...
                for (
                    key,
                    _value,
                ) in ROE_data.items():  # 先算出股票的買賣訊號 --已拆分IBacktestSignal
            ...
            if has_trade:
                if len(buy_numbers) != 0:
                    buy_data = pd.concat(
                        [buy_data, pd.DataFrame({"Date": index, "code": buy_numbers})],
                        ignore_index=True,
                    )
                if len(sell_numbers) != 0:
                    sell_data = pd.concat(
                        [
                            sell_data,
                            pd.DataFrame({"Date": index, "code": sell_numbers}),
                        ],
                        ignore_index=True,
                    )
    ...
    def backtest_Record_high_Fast(self, mainParament: RecordBackTestParameter):
        ...
        for index, row in All_data.iterrows():
            ...
            if len(userInfo.HandleStock) > 0:
                ...
                for key, value in list(Temp_data.items()):
                    if GetStockData().get_stock_price(
                        key, userInfo.BaseInfoData.now_day, stock_data_kind.AdjClose
                    ) < GetStockData.get_stock_MA(
                        key, userInfo.BaseInfoData.now_day, 20
                    ):
                        sell_stock(key, value.Amount)
    ...
    def backtest_PERandPBR_Fast(self, mainParament: RecordBackTestParameter):
        ...
        for index, row in All_data.iterrows():
            ...
            if (
                ...
                Temp_buy = Tools.MixDataFrames(Temp_buy0)
                if (
                    Temp_buy0.__contains__("price")
                    and Temp_buy0["price"].empty == False
                ):
                    Temp_buy = Temp_buy.sort_values(by="price", ascending=False)
                if (
                    Temp_buy0.__contains__("volume")
                    and Temp_buy0["volume"].empty == False
                ):
