...
def BacktestSignalFactory(_StrategyType: BacktestSignalType, _originalStock: OriginalStock) -> IBacktestSignal:
...
class IBacktestFilter(ABC):
    ...
    def RunFilter(self, Date: datetime) -> pd.Series:
        raise NotImplementedError("{} is virtual! Must be overwritten.".format(sys._getframe().f_code.co_name))
def BacktestFilterFactory(_backtestFilterType: BacktestFilterType, _indicator: Indicator) -> IBacktestFilter:
