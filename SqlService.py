...
class SqlService():
    ...
    def readStockDay(self, name:str):
        ...
        try:
            with self.server_flask.app_context():
                dataframe = pd.read_sql(sql = name,con=self.MySql_server.engine,index_col='Date')
                return dataframe
    ...
    def yfInfo(self, name:str):
        ...
        df = data.get_data_yahoo([name], start_date, end_date)
        if df.empty:
            print("yahoo no data:" + str(name))
        else:
            with self.server_flask.app_context():
                df.to_sql(name=name,con=self.MySql_server.engine,if_exists='replace')
    ...
    def __SetMysqlServer(self, db_name:str):
        ...
        self.server_flask.config['SQLALCHEMY_DATABASE_URI'] = "mysql+pymysql://" + "demo" + ":" + "~Demo123" + "@" + "127.0.0.1" + ":"+ "3307" +"/"+ str(db_name)
        #連線mysql DB
