...
def backtest_monthRP_Up(change,AvgMon,UpMon,Start_date,End_date,start_money,PER_start,PER_end,
    ...
    Temp_alldata.to_csv('backtestAll.csv')
#定期定額
def backtest_Regular_quota(buy_day,buy_money,number,start_date,end_date):
    userInfo = get_user_info.data_user_info(0,start_date,end_date)
    buy_month = start_date
    Temp_result_pick = pd.DataFrame(columns=['date','選股數量'])
    while userInfo.now_day <= userInfo.end_day:
        ...
        #入場訊號篩選--------------------------------------
        if userInfo.now_day.month != buy_month.month and userInfo.now_day.day >= buy_day:
            userInfo.now_money = userInfo.now_money + buy_money
            userInfo.start_money = userInfo.start_money + buy_money
            Temp_price = get_stock_history.get_stock_price(number,userInfo.now_day,get_stock_history.stock_data_kind.AdjClose)
            Temp_stockNumber = int(buy_money/Temp_price)
            userInfo.buy_stock(number,Temp_stockNumber)
