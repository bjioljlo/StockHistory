"""
UpdateStockService Module - Main Service Facade
...
class UpdateStockService:
    def __init__(self, sql_service: SqlService, mongo_service: MongoService, read_load_system: ReadLoadSystem, config: dict = None, cache_service=None) -> None:
        ...
        # Always create data validator, use default config if not provided
        ...
        # ADL update configuration
        self._adl_update_enabled = self._data_validator.config.get('app', {}).get('auto_adl_update', True)
        self._adl_update_timeout = self._data_validator.config.get('app', {}).get('adl_update_timeout', 300)  # 5 minutes
        
        # Initialize specialized components
        self._downloader = StockDataDownloader(self._retry_attempts, self._retry_delay)
        self._synchronizer = StockDataSynchronizer(self._sql_service, self._mongo_service)
    ...
    def UpdateMongoHandle(self, callback=None):
        ...
        for i, table_name in enumerate(all_tables):
            self._synchronizer.sync_table_to_mongo(table_name)
            if callback:
    ...
    def _download_with_retry(self, stock_symbol: str, start_date: datetime, end_date: datetime, tz: str = None) -> pd.DataFrame:
        return self._downloader.download_with_retry(stock_symbol, start_date, end_date, tz)

    def _sync_table_to_mongo(self, table_name: str):
        self._synchronizer.sync_table_to_mongo(table_name)

    def _replace_stock_data(self, stock_name: str, df_result: pd.DataFrame) -> bool:
        return self._synchronizer.replace_stock_data(stock_name, df_result)

    def _upsert_stock_data(self, stock_name: str, df_result: pd.DataFrame) -> bool:
        return self._synchronizer.upsert_stock_data(stock_name, df_result)

    def _determine_market(self, stock_name: str) -> str:
        return self._downloader.determine_market(stock_name)

    def _basic_data_cleanup(self, df: pd.DataFrame, stock_code: str) -> pd.DataFrame:
        return self._downloader.basic_data_cleanup(df, stock_code)

    def _persist_adl_data(self) -> bool:
        return self._adl_updater.persist_adl_data()
    ...
    def __RunUpDateADL(self, callback=None):
        return self._adl_updater.run_update_adl(callback)