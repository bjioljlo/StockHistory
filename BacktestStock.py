        ...
        for index,row in Temp_table.iterrows():
            ...
            if has_trade:
                ...
                if len(sell_numbers) != 0:
                    sell_data = pd.concat([sell_data,pd.DataFrame({'Date':index,'code':sell_numbers})],ignore_index = True)
    ...
    def backtest_Record_high_Fast(self, mainParament:RecordBackTestParameter):
        ...
        for index,row in All_data.iterrows():
            ...
            if len(userInfo.HandleStock) > 0:
                Temp_data = userInfo.HandleStock
                for key,value in list(Temp_data.items()):
                    if GetStockData().get_stock_price(key,userInfo.BaseInfoData.now_day,stock_data_kind.AdjClose) < GetStockData.get_stock_MA(key,userInfo.BaseInfoData.now_day,20):
                        sell_stock(key,value.Amount)
                        has_trade = True
            #開始篩選--------------------------------------
